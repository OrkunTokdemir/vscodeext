...
export async function activate(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    logger.info(`Activation done: ${consts.EXTENSION_ID}`);
  } catch (e) {
    logger.error(`Activation failed: ${consts.EXTENSION_ID}: ${String(e)}`);
  }
}
...
async function initCoreApi() {
  ...
  if (!coreApi) {
    throw new Error('Failed to get CoreAPI');
  }
}
...
const onPyApiEnvChanged = async (e: PyApiEnvChanged) => {
  logger.info(`Active env changed: ${e.resource?.uri.fsPath}`);
