...
{/snippet}
<!--
{#snippet QmlTraceViewer()}
  <Column class='!-space-y-2'>
    <Row class='items-center '>
      {@render ItemLabel('QML trace viewer (global)')}
      {#if data.configs.viewer.valid}
        <Info class='text-orange-500'/>
      {/if}
    </Row>
    ...
    <Row class='items-center'>
      <div class='grow'>
        <InputWithIssue
          disabled
          bind:value={data.configs.viewer.path}
          class='grow'
        />
      </div>
      <IconButton
